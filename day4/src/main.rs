...
impl Bingoboard {
    ...
    fn check_bingo(&mut self) -> bool {
        ...
        let bingo_row = self
            .data
            .iter()
            .filter(|row| row.into_iter().filter(|index| index.1).count() == 5)
            .count()
            == 1;
        //columns
    ...
    fn score(&mut self, winning_num: u8) -> u64 {
        let mut score: u64 = 0;
        for row in self.data.iter() {
            let row_score: u64 = row.into_iter().fold(0, |sum, elem| {
                if !elem.1 {
...
fn parse_input(input: &str) -> (Vec<u8>, Vec<Bingoboard>) {
    ...
    let bingoboards = lines
        ...
        .map(|parsed_chunk| Bingoboard { data: parsed_chunk })
        .collect();
